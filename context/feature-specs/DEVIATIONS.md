...
- `[FEAT-008-BE]` — Next.js 16 `revalidateTag` profile signature requirement — Next.js 16 requires a second argument (`profile: string | CacheLifeConfig`) for `revalidateTag(tag, profile)`. Passed `"hours"` to align with the storefront cached catalog read configuration (`cacheLife("hours")` in `products.ts`).
- `[ISSUE-014]` — Next.js 16 `revalidateTag` single-argument reconciliation — Per ISSUE-014, updated `revalidateTag("products", "hours")` calls to single-argument `revalidateTag("products")`. Augmented Next.js module types in `src/types/next-cache.d.ts` to allow the single-argument signature without TypeScript errors while preserving runtime cache invalidation.
- `[FEAT-009-BE]` — Automatic refund status on cancel & paid revenue summation — When transitioning order status to `CANCELLED` on an order where `paymentStatus === "PAID"`, automatically marked `paymentStatus: "REFUNDED"` and logged an administrative notice; and calculated `totalRevenue` strictly as the sum of subtotals of paid orders per acceptance criteria line 68.
- `[FEAT-010-BE]` — Next.js 16 dynamic route collision between `/api/products/[slug]` and `/api/products/[id]/reviews` — Placed reviews endpoints under `/api/products/[slug]/reviews` and `/api/products/[slug]/reviews/summary` with dual parameter resolution (`params.slug || params.id` matching both internal product IDs and public slugs) to prevent Next.js App Router ambiguous dynamic segment compile collisions.
